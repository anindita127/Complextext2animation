...
def sample(args, exp_num, data=None):
    ...
    model = eval(args.model)(chunks, input_size,
    ...
    def loop(model, data, dataLoaders, pre, batch_size, desc='train'):
        ...
        total_var_gt = [trajectory_var, pelvis_var, torso_var, lowerneck_var,  upperneck_var,
                        leftshoulder_var, leftelbow_var, leftwrist_var, rightshoulder_var, rightelbow_var,
                        rightwrist_var, lefthip_var, leftknee_var, leftankle_var, leftfoot_var, righthip_var,
                        rightknee_var, rightankle_var, rightfoot_var, root_var]

        return total_loss, total_var, total_var_gt
    ...
    test_loss_list, varlist, var_gtlist = loop(
    ...
    vari = [x / ntest for x in varlist]
    with open(dir_name+"/test_variance.bin", "wb") as fp:
        pickle.dump(vari, fp)
    var_gt = [x / ntest for x in var_gtlist]
    with open(dir_name+"/gt_test_variance.bin", "wb") as fp:
        pickle.dump(var_gt, fp)
