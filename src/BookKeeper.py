...
class BookKeeper():
  ...
  def print_res(self, epoch, key_order=['train', 'dev', 'test'], metric_order=[], exp=0, lr=None, fmt='{:.9f}'):
  ...
  def print_res_archive(self, epoch, key_order=['train', 'dev', 'test'], exp=0, lr=None, fmt='{:.9f}'):
  ...
  def _load_model(self, model):
    ...
    if isinstance(model, torch.nn.DataParallel):
      model.module.load_state_dict(torch.load(open(weights_path, 'rb')))
    else:
      t = torch.load(open(weights_path, 'rb'))
