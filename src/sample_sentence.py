import torch
import torch.nn as nn
from re import search
from difflib import SequenceMatcher
from dataUtils import *
from model.model import *
from model.model_hierarchical_twostream import *
from data import *
from common.transforms3dbatch import *
from renderUtils import parallelRender
from BookKeeper import *
from argsUtils import argparseNloop
from slurmpy import Slurm
import os
import numpy as np
from tqdm import tqdm
import pdb
import spacy

# Load English tokenizer, tagger, parser, NER and word vectors
# nlp = spacy.load("en_core_web_sm")


def similar(a, b):
    return SequenceMatcher(None, a, b).ratio()


def toFKE(M, data, filename):
    ''' Convert RIFKE to FKE '''
    output_columns = data.raw_data.output_columns('rifke')
    M = data.raw_data.rifke2fke(M[output_columns].values)

    ''' Save FKE '''
    M = M.reshape(M.shape[0], -1)
    output_columns = data.raw_data.output_columns('fke')
    pd.DataFrame(data=M,
                 columns=output_columns).to_csv(filename.as_posix())


def sample(args, exp_num, data=None):
    assert args.load, 'Model name not provided'
    assert os.path.isfile(args.load), 'Model file not found'
    args_subset = ['exp', 'cpk', 'model', 'time']
    book = BookKeeper(args, args_subset, args_dict_update={})
    args = book.args

    dir_name = book.name.dir(args.save_dir)
    # dir_name = args.load[:-2]

    # Training parameters
    path2data = args.path2data
    dataset = args.dataset
    lmksSubset = args.lmksSubset
    desc = args.desc
    split = (args.train_frac, args.dev_frac)
    idx_dependent = args.idx_dependent

    # hardcoded for sampling
    batch_size = 1
    time = 0
    chunks = args.chunks
    offset = args.offset
    # mask for delta
    mask = args.mask

    global feats_kind
    feats_kind = args.feats_kind
    global render_feats_kind
    render_feats_kind = {'rifke': 'fke',
                         'quaternion': 'quaternion',
                         'fke': 'fke'}
    s2v = args.s2v
    f_new = args.f_new
    curriculum = args.curriculum

    # Load data iterables
    if data is None:
        data = Data(path2data, dataset, lmksSubset, desc,
                    split, batch_size=batch_size,
                    time=time,
                    chunks=chunks,
                    offset=offset,
                    shuffle=False,
                    mask=mask,
                    feats_kind=feats_kind,
                    s2v=s2v,
                    f_new=f_new)
        print('Data Loaded')
    else:
        print('Data already loaded!! Yessss!')

    train = data.train.dataset.datasets
    # dev = data.dev.dataset.datasets
    # test = data.test.dataset.datasets

    # Create a model
    global device
    # device = torch.device('cuda:{}'.format(args.cuda)
    #                       ) if args.cuda >= 0 else torch.device('cpu')
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    input_shape = data.input_shape
    modelKwargs = {}
    modelKwargs.update(input_shape)
    modelKwargs.update(args.modelKwargs)

    # getting the input_size
    input_size = 300

    model = PoseGenerator(chunks, input_size=input_size,
                          Seq2SeqKwargs=modelKwargs, load=None)
    model.to(device).double()
    # discriminator = Discriminator().to(device).double()
    print('Model Created')

    # Load model
    if args.load:
        print('Loading Model')
        book._load_model(model)
        # checkpoint = torch.load(args.load)
        # model.load_state_dict(checkpoint['model'])

    # Transforms
    global columns
    columns = get_columns(feats_kind, data)
    # args.transforms = ['quat']
    pre = Transforms(args.transforms, columns, args.seed,
                     mask, feats_kind, dataset, f_new)

    def loop(model, data, loader, sentences, pre, batch_size, desc='train'):
        filenames = []
        output_filenames = []
        descriptions = sentences
        model.eval()
        description = data.df['descriptions']
        for count, s2v in enumerate(tqdm(sentences)):
            filename = Path(dir_name)/Path('new')/Path('{}.csv'.format(count))
            # filenames.append(filename.with_suffix('.fke').as_posix())
            output_filenames.append(filename.with_suffix('.mp4').as_posix())
            os.makedirs(filename.parent, exist_ok=True)
            # try:
            # doc = nlp(s2v)
            # stripped_current = ""
            # for token in doc:
            #     if token.dep_ != "nsubj" and \
            #         (token.pos_ == "NOUN"  or \
            #         token.pos_ == "VERB" or \
            #         token.pos_ == "ADV" or \
            #         token.pos_ == "ADJ" or \
            #         token.text == 'and' or \
            #         token.text == 'while'    ) :
            #         stripped_current += ' ' + str(token.lemma_)

            # split_sent.append(stripped_current[1:].split(' '))
            # stripped_sent = stripped_current[1:]
            ratio = np.zeros(len(description))
            # maxratio = 0
            # max_index = 0
            for i, value in description.items():
                ratio[i] = similar(description[i], s2v)
            ratio_sort = np.sort(ratio)[::-1]
            max_indices = np.argsort(ratio)[::-1]
            row = data.df.iloc[max_indices[0]]
            minidata = MiniData(row[feats_kind], args.lmksSubset, args.time,
                                sentence_vector=row['descriptions'],
                                mask=args.mask,
                                dataset=args.dataset,
                                f_ratio=int(data.raw_data._get_f()/args.f_new),
                                feats_kind=feats_kind)
            batch = minidata[0]
            start = batch['input']
            print(batch['desc'])
            start = np.concatenate([start[1], start[0]], axis=-1)
            start = pre.transform(torch.from_numpy(
                start).to(device).double())[0, 0:1]

            pose_decoder, language_ = model.sample(
                [s2v], time_steps=70, start=start)

            outputs = pre.inv_transform(pose_decoder).squeeze(0)
            new_size = list(outputs.shape)
            new_size[0] *= loader[0].f_ratio
            outputs = outputs.repeat(1, loader[0].f_ratio).view(new_size)
            outputs = outputs.detach().cpu().numpy()
            # copy outputs in the dataframe format
            mat_full_temp = pd.DataFrame(data=np.zeros(
                (outputs.shape[0], len(columns))), columns=columns)

            # mat_full_temp = pd.DataFrame(outputs)
            if feats_kind == 'rifke':
                mat_full_temp['root_tx'] = 0
                mat_full_temp['root_tz'] = 0

            # copy all joints
            mat_full_temp.loc[:, columns] = outputs

            if feats_kind == 'quaternion' or feats_kind == 'axis-angle':
                data.raw_data.mat2csv(mat_full_temp.values, filename, columns)
            elif feats_kind == 'rifke':
                output_columns = data.raw_data.output_columns('rifke')
                mat_full_temp[output_columns].to_csv(
                    filename)  # save rifke as well
                # toFKE(mat_full_temp,
                #       data,
                #       filename.with_suffix('.fke'))

    with open('sentences.txt', 'r') as f:
        sentences = f.readlines()
    sentences = [s.strip() for s in sentences]
    loop(model, data, train, sentences, pre, batch_size, 'train')


if __name__ == '__main__':
    argparseNloop(sample)
